...
exports.get = function (id, cb) {
	var query = 'SELECT games.id AS id, games.winning_address AS winning_address , games.winner AS winner  , users.id AS users_name FROM games LEFT JOIN users ON games.winner = users.id WHERE games.id = $1';
...
exports.getAll = function (start, end, cb) {
  start = 0;
	...
	var query = 'SELECT games.id AS id, games.winning_address AS winning_address , games.winner AS winner  , users.id AS users_name FROM games LEFT JOIN users ON games.winner = users.id WHERE games.id >= $1 AND games.id <= $2';
